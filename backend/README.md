...
**Plan changes:** Upgrading keeps existing Dock Keys and ad-hoc tables. **Downgrading** (moving to a tier with fewer seats) immediately revokes all OBS Dock Keys and deletes all ad-hoc tables so the account must rebuild within the new limits. Match history is kept.
