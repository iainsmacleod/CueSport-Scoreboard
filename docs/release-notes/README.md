...
node capture-screenshots.mjs --version 8.2.4
git add 8.2.4/images/
git commit -m "Update 8.2.4 release note screenshots"
```
...
```bash
node capture-screenshots.mjs --version 8.2.4 --cloud http://localhost:4003
```
...
```powershell
.\docs\release-notes\create-release.ps1 -Version 8.2.4
```

Or manually:

```bash
gh release create v8.2.4 \
  --draft \
  --title "CueSport Scoreboard 8.2.4" \
  --notes-file docs/release-notes/8.2.4/GITHUB_RELEASE_DRAFT.md
```
