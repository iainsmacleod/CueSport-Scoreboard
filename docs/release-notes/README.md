...
node capture-screenshots.mjs --version 8.2.3
git add 8.2.3/images/
git commit -m "Update 8.2.3 release note screenshots"
```
...
```bash
node capture-screenshots.mjs --version 8.2.3 --cloud http://localhost:4003
```
...
```powershell
.\docs\release-notes\create-release.ps1 -Version 8.2.3
```

Or manually:

```bash
gh release create v8.2.3 \
  --draft \
  --title "CueSport Scoreboard 8.2.3" \
  --notes-file docs/release-notes/8.2.3/GITHUB_RELEASE_DRAFT.md
```
